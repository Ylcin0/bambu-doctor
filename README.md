...
| **AI** | 只负责"图里是什么现象" | 感知可以靠模型，推理不能 |

如果让模型一把梭"看照片给建议"，结论不可测、不可积累，而且它会自信地胡说。所以即使以后接上视觉模型，它也**只能**把照片翻译成一个预定义的症状 ID，剩下的判断还是走规则。

### 透明件：一条完整的规则线

透明件的目标不是"打得干净"，而是**让光穿过去**——所以参数方向跟平常常常相反（流量宁高勿低、风扇要关、墙要少）。这条线已经内置了：

```
$ bambu-doctor diagnose "发雾不透光" --profile 透明 --process 透明档

【判断】下面这些根因，在你的参数里是成立的

  1. 冷却风扇没关（透明件要求全关）
     依据：你的 fan_max_speed = 50（高于上限 1）
     动作：把这卷料的 fan_max_speed 和 fan_min_speed 都设为 0
     （Bambu Studio：耗材设置 → 冷却）

  2. 填充密度不足（透明件要 100%）
     依据：你的 sparse_infill_density = 15（低于下限 99）

  3. 层高偏大（透明件建议 0.1mm）
     依据：你的 layer_height = 0.2（高于上限 0.15）

  4. 流量比偏低（透明件要往高调）
     依据：你的 filament_flow_ratio = 0.94752（低于下限 0.96）

【需要你自己查】工具读不到，但你两分钟能确认
  · 耗材受潮 —— 受潮是发雾的头号原因，排在所有参数问题之前，先排除它
```

注意这条输出同时读了**三类 profile**：`fan_max_speed` 在耗材档、`sparse_infill_density` 和 `layer_height` 在工艺档、机型在机器档。Bambu 的参数就是这么分散的——只读一类，就会把"定义在别处的参数"误判成"没设置"。

依据是 Bambu Lab 官方的透明打印指南。每条规则的"注意"里都写明了它的可信度：比如温度那条标的是"只是经验值"，不会让它看起来像"官方数值"。
